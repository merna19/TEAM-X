...
/* UART initialization */
E_Errore_State_UART MCAL_UART_init(uint32 baudRate)
{
	...
	else
	{
		...
		uint32 BRValue = (F_CPU/(16UL*baudRate)) - 1;

		/* Set baud rate */
		UCSRC &= ~(1<<URSEL);
		UBRRH = (uint8)(BRValue>>8);
		UBRRL = (uint8)BRValue;
...
E_Errore_State_UART MCAL_UART_send_sysch(uint8 Data)
{
	E_Errore_State_UART error =OK;
	if(UART_STATUS_ERROR==NOT_OK)
	{
